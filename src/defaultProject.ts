...
const image = (name: string, src: string, x: number, y: number, w: number, h: number, z = 4, opacity = 1): DesignElement => ({
  id: uid('img'), kind: 'image', name, src, alt: name, x, y, w, h, rotation: 0, z,
...
export function pageFromTemplate(kind: PageKind, project: Pick<BookProject, 'settings'>): BookPage {
  ...
  if (kind === 'cover') {
    ...
    p.elements.push(image('카세트', asset('cassette.svg'), frontX + 70, b + 120, 66, 43, 5, .94));
    p.elements.push(image('헤드폰', asset('headphones.svg'), frontX + 8, b + 136, 45, 45, 4, .84));
    p.elements.push(image('뒷표지 파형', asset('waveform.svg'), b + 18, b + 142, tw - 36, 22, 3, .62));
    ...
    p.elements.push(image('헤드폰', asset('headphones.svg'), 13, 137, 50, 50, 4, .88));
    p.elements.push(image('카세트', asset('cassette.svg'), 72, 135, 66, 43, 5, .96));
    p.elements.push(image('파형', asset('waveform.svg'), 18, 181, size.widthMm - 36, 18, 3, .62));
    ...
    p.elements.push(text('시리즈', 'PSALM NEXT 01', 18, 203, size.widthMm - 36, 10, 8, '#857d70'));
  } else if (kind === 'half-title') {
    p.name = '반표제지';
    p.elements.push(image('작은 파형', asset('waveform.svg'), 48, 72, 56, 12, 3, .55));
    ...
    p.name = '속표지';
    p.elements.push(image('헤드폰', asset('headphones.svg'), 58, 151, 36, 36, 3, .65));
    p.elements.push(image('파형', asset('waveform.svg'), 34, 188, 84, 14, 3, .5));
    ...
    p.name = '속표지 뒷장 · 판권';
    p.background = '#fbfaf6';
    p.elements.push(image('희미한 파형', asset('waveform.svg'), 30, 182, 92, 18, 1, .22));
    ...
    p.name = 'TRACK 페이지';
    p.background = '#faf7f0';
    p.elements.push(text('트랙 번호', 'TRACK 01', 16, 16, 52, 12, 9, '#7a867d'));
    p.elements.push(text('트랙 제목', '원본 그대로', 16, 30, size.widthMm - 32, 20, 22, '#2c332e'));
    const psalm = text('시편', 'PSALM 139', 16, 54, 35, 10, 8, '#ffffff');
    psalm.style.fill = '#66796e'; psalm.style.borderRadiusMm = 5; psalm.style.textAlign = 'center'; p.elements.push(psalm);
    p.elements.push(image('파형', asset('waveform.svg'), 58, 50, 76, 14, 4, .72));
    p.elements.push(image('플레이어', asset('player.svg'), 16, 68, size.widthMm - 32, 24, 5, .96));
    p.elements.push(text('기분 체크', '기분 체크   ○ 무난함   ○ 눈치 보임   ○ 지침   ○ 궁금함   ○ 그냥그럼', 16, 96, size.widthMm - 32, 15, 10, '#3d433f'));
    const liner = text('라이너 노트', 'LINER NOTE\n우리는 하루 종일 보정하며 살아요. 그런데 시편은 보정 전 원본으로 하나님 앞에 서는 법을 가르쳐 줍니다. 실제 원고를 넣고, 그래픽 자산과 실제 텍스트 조판을 분리해 샘플 이미지의 감성을 유지하세요.', 16, 116, size.widthMm - 32, 48, 10.5, '#343a36');
    liner.style.fill = '#eee8dc'; liner.style.borderRadiusMm = 4; p.elements.push(liner);
    const lyrics = text('내 가사 쓰기', 'MY LYRICS\n하나님, 사실 저의 원본은 ________________________________', 16, 170, size.widthMm - 32, 27, 10.5, '#343a36');
    lyrics.style.fill = '#e7eee8'; lyrics.style.borderRadiusMm = 4; p.elements.push(lyrics);
    p.elements.push(text('기도', 'PRAYER  보정 없는 저를 이미 아시는 하나님, 오늘 그 말씀을 믿게 해 주세요. 아멘.', 16, 203, size.widthMm - 32, 12, 8.5, '#596a60'));
  } else if (kind === 'back') {
    p.name = '전자책 뒷표지';
    p.background = '#5e7065';
    p.elements.push(image('뒷표지 파형', asset('waveform.svg'), 20, 152, size.widthMm - 40, 22, 3, .25));
    ...
    if (kind === 'chapter') p.elements.push(image('헤드폰', asset('headphones.svg'), 92, 130, 44, 44, 3, .58));
